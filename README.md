...
#### BWA
To align the paired end Illumina DNA reads to the canu assembled genome, BWA was used according to the script "02_bwa_alignment.sh" located at "code/02_bwa_pacbio_illumina/". Here the commands bwa aln and bwa sampe were used, which generates alignments given paired-end data. However, I now realize that bwa mem would have been a better choice for the alignment due to it being more accurate, according to the BWA manual. I am not sure to what extent this might have affected my assembly.

#### Pilon
To improve the canu assembly, Pilon was used according to the script "03_pilon_polishing.sh" located at "code/03_pilon_polishing/". This step is performed since Illumina reads have higher accuracy than PacBio reads. There are not many choices to be made when using Pilon, or any outputed statistics so there is not much to discuss about this particular step. 
...
In order to do the differential expression analysis, star was once again used to map the RNA transcripts to the reference assembly with the scripts "starDE.sh" and "star_aril2.sh" located at "code/10_star_DE/". This time, the gtf file produced by braker was supplied when creating the genome index. The option sjdbOverhang was set to 100 since the optimal value for this parameter is calculated as "Max(readlength)-1", and the longest RNA read in the sequence files was 101 basepairs long. This time 1 bam file was produced for each sample used, where the script "starDE.sh" was used for creating one alignment with the root and one with the aril. "star_aril2.sh" was used for aligning a second RNA sequence file from a fruit aril when realising multiple replicates would work better with DESeq2. The SRR accession numbers for the samples used are SRR6040093 (root), SRR6040094 (aril), and SRR6040097 (aril).

#### Read counting using HTSeq
To count the reads mapping to each gene, HTSeq-count was used according to the scripts "htseq_aril.sh", "htseq_aril_097.sh" and "htseq_root.sh" located at "code/11htseq/". The original gtf-file produced by braker did not work for HTSeq-count since some rows did not have the "transcript_id" or "gene_id" attributes in the last column. In order to make it work, all lines with the attribute "transcript_id" were extracted using grep and put into a new file like this: 
```bash
...
Using DESeq, a log2 fold change plot with noise from low counts removed, and a heatmap showing the genes with the highest variance of normalized readcounts between samples, was created as seen below. 
