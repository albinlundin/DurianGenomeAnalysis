...
In order to do the differential expression analysis, star was once again used to map the RNA transcripts to the reference assembly with the scripts "starDE.sh" and "star_aril2.sh" located at "code/10_star_DE/". This time, the gtf file produced by braker was supplied when creating the genome index. The option sjdbOverhang was set to 100 since the optimal value is calculated as "Max(readlength)-1", and the longest RNA read in the sequence files was 101 basepairs long. This time 1 bam file was produced for each sample used, where the script "starDE.sh" was used for creating one alignment with the root and one with the aril. "star_aril2.sh" was used for aligning a second RNA seq file for a fruit aril when realising multiple replicates would work better with DESeq2.
